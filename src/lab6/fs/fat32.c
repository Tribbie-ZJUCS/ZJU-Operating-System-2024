...
void fat32_init(uint64_t lba, uint64_t size) {
    ...
    fat32_volume.first_data_sec = fat32_volume.first_fat_sec + fat32_header.fat_sz32 * fat32_header.num_fats/* to calculate */;
...
int64_t fat32_lseek(struct file* file, int64_t offset, uint64_t whence) {
    if (whence == SEEK_SET) {
        ...
        file->cfo = offset + get_file_size(file);/* to calculate */;
    } else {
